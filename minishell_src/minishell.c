...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/02 20:14:26 by bandre            #+#    #+#             */
/*   Updated: 2018/05/02 20:22:02 by bandre           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static const t_command g_list_command[] = {
	{ "setenv", ft_setenv },
	{ "env", ft_env },
...
int		testcommand(char **command, t_libft_chained_list **env, char *path)
{
	...
	while (i < 7)
	{
		if (ft_strcmp(command[0], g_list_command[i].name) == 0)
		{
			g_list_command[i].shell_function(env, path, command);
			return (1);
		}
		i++;
	}
